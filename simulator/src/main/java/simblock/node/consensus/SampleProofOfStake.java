...
public class SampleProofOfStake extends AbstractConsensusAlgo {
  ...
  public SampleStakingTask minting() {
    ...
    System.out.print("block mining time: " + BigDecimal.valueOf((Math.log(u) / Math.log( 1.0 - p) * 1000)).toBigInteger().toString());
    return new SampleStakingTask(selfNode, BigDecimal.valueOf((Math.log(u) / Math.log( 1.0 - p) * 1000)).toBigInteger(), difficulty);
