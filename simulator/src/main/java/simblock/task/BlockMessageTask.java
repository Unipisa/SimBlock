...
public class BlockMessageTask extends AbstractMessageTask {
  ...
  public BlockMessageTask(Node from, Node to, Block block, long delay) {
    super(from, to);
    this.block = block;
    this.interval = getLatency(this.getFrom().getRegion(), this.getTo().getRegion()).add(BigInteger.valueOf(delay));
  }
  ...
  public void run() {
    ...
    //These lines must be uncommented if SimBlock Visualizer needs to be used.
    //OUT_JSON_FILE.print("{");
