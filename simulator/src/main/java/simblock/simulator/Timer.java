...
public class Timer {
  ...
  private static BigInteger currentTime = new BigInteger ("0");
  ...
  private static class ScheduledTask implements Comparable<ScheduledTask> {
    ...
    private final BigInteger scheduledTime;
     ...
     */
    private ScheduledTask(Task task, BigInteger scheduledTime) {
      this.task = task;
     ...
     */
    private BigInteger getScheduledTime() {
    ...
    public int compareTo(ScheduledTask o) {
      ...
      BigInteger timeDiff = this.scheduledTime.subtract(o.scheduledTime);
      int order = timeDiff.signum();
      if (order != 0) {
        return order;
      }
  ...
  public static void putTask(Task task) {
    ScheduledTask scheduledTask = new ScheduledTask(task, currentTime.add(task.getInterval()));
  ...
  public static void putTaskAbsoluteTime(Task task, BigInteger time) {
