...
public class Main {
  ...
  public static void main(String[] args) {
    ...
    while (getTask() != null) {
      if (getTask() instanceof AbstractMintingTask) {
        ...
        // This must be uncommented if SimBlock Visualizer needs to be used
    ...
    blockList.sort((a, b) -> {
      BigInteger timeDifference = a.getTime().subtract(b.getTime());
      int order = timeDifference.signum();
      if (order != 0) {
        return order;
      }
    ...
    //The following lines must be uncommented to print (height of the) orphan blocks
    //for (Block orphan : orphans) {
    //  System.out.println(orphan + ":" + orphan.getHeight());
    //}

    //System.out.print("averageOrphanSize: ");
    ...
    // TODO move to method and use logger
    try {
      FileWriter fw = new FileWriter(new File(OUT_FILE_URI.resolve("./blockList_" + LocalDateTime.now().toString() + ".txt")), false);
    ...
    //The folllowing lines must be uncommented if SimBlock Visualizer needs to be used.
    //OUT_JSON_FILE.print("{");
  ...
  public static int genMiningPower() {
    ...
    //int max = Math.max((int) (r * STDEV_OF_MINING_POWER + AVERAGE_MINING_POWER), 1);

    //System.out.print("mining power: " + Math.max((int) (r * STDEV_OF_MINING_POWER + AVERAGE_MINING_POWER), 1));
