...
public class Network {
   ...
   * Return BigInteger datatype instead of long datatype
   * @param from the from latency
   * @param to   the to latency
   * @return the calculated latency
   */
  public static final BigInteger getLatency(int from, int to) {
    long mean = LATENCY[from][to];
    double shape = 0.2 * mean;
    double scale = mean - 5;
    return BigInteger.valueOf(Math.round(scale / Math.pow(random.nextDouble(), 1.0 / shape)));
  }
